...
def torch_block_wise_attention(
    ...
    mask = torch.zeros(
        b, h, num_block, num_block, dtype=torch.bool, device=q.device
    )
    mask[
...
def triton_column_count_cumsum(
    x: torch.Tensor, num_columns: int
) -> torch.Tensor:
...
def triton_block_wise_prefill_attention(
    ...
    k: torch.Tensor,
    v: torch.Tensor,
    block_idx: torch.Tensor | list[list[torch.Tensor]],
    block_size: int,
    grid_offset: int = 0,
    softmax_scale: float | None = None,
    gqa_interleave: bool = False,
...
def score_cover_topk(x: torch.Tensor, score: float):
    cumsum_x = torch.cumsum(
        torch.sort(x, dim=-1, descending=True).values, dim=-1
    )
...
def transform_vertical_slash_idx(v_idx, s_idx, num_blocks):
    ...
    range_blocks = torch.arange(num_blocks, device=s_idx.device)[
        None, None, :, None
    ]
    # vertical
    v_idx = (
...
def square_root_js_divergence(p: torch.Tensor, q: torch.Tensor):
    m = (p + q) / 2
    return torch.sqrt(
        0.5 * (p * torch.log(p / m)).sum(-1)
        + 0.5 * (q * torch.log(q / m)).sum(-1)
    )
...
def get_active_blocks(
    ...
    slash_topk[
        torch.arange(slash_topk.shape[-1], device=num_slash_blocks.device)[
            None, :
        ]
    ...
    for b, h in block_sparse_mask.nonzero():
        if block_causal_mask is None:
            block_causal_mask = torch.tril(
                torch.ones(
                    num_blocks, num_blocks, device=q.device, dtype=torch.bool
                )
            )
...
def calculate_average_sparsity(block_idx, batch_size, num_heads, num_blocks):
    ...
    for b in range(batch_size):
        for h in range(num_heads):
            ...
            if isinstance(selected_blocks, torch.Tensor):
