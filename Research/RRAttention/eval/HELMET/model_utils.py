    ...
    def generate_batch(
        ...
        **kwargs,
        ...
        if inputs is None:
            ...
            for i in tqdm(inputs, desc=desc):
...
class HFModel(LLM):
    ...
    def generate(self, inputs=None, prompt=None, **kwargs):
        ...
        if record_e2e_ms:
            ret["e2e_ms"] = e2e_elapsed_time_ms
        if record_ttft_ms:
            ret["ttft_ms"] = ttft_elapsed_time_ms
        if record_attn_ms:
            ret["attn_ms"] = attn_ms
            ret["estimate_func_ms"] = estimate_func_ms
        return ret
...
def _load_generation_eos_token_ids(model_name_or_path, tokenizer):
    ...
    return _normalize_token_ids(eos_token_ids)
...
class PaddleWorkerModel(LLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_length = max_length
        self.paddle_model_name_or_path = model_name
        self.hf_model_name_or_path = model_name
        self.model_type = kwargs.get("model_type", "auto")
        self.method = kwargs.get("method", "full")
    ...
    def _start_worker(self):
        ...
        cmd = [
            ...
            str(worker_path),
            "--model_name_or_path",
            self.paddle_model_name_or_path,
            "--model_type",
            self.model_type,
            "--method",
            self.method,
        ...
        if self.worker_device is not None:
            cmd.extend(["--device", self.worker_device])
        if self.tiny_random:
            cmd.append("--tiny_random")
        if self.clear_cache_per_request:
            cmd.append("--clear_cache_per_request")

        env = os.environ.copy()
        root_dir = str(Path(__file__).resolve().parents[2])
        env["PYTHONPATH"] = root_dir + os.pathsep + env.get("PYTHONPATH", "")
        env.setdefault("PYTHONUNBUFFERED", "1")
        logger.info(f"Starting Paddle worker: {' '.join(cmd)}")
    ...
    def _terminate_worker(self, timeout=5):
    ...
    def _worker_request(self, payload):
        ...
        def _readline(stdout, output_queue):
            try:
                output_queue.put(("line", stdout.readline()))
            except Exception as exc:
                output_queue.put(("error", exc))

        reader = threading.Thread(
            target=_readline,
            args=(self._worker.stdout, result_queue),
            daemon=True,
        )
        reader.start()
        ...
        while True:
            ...
            if remaining <= 0:
                ...
                raise RuntimeError(
                    f"Paddle worker timed out after {self.worker_timeout_s:.1f}s while handling request {payload.get('id')}"
                )
            try:
                kind, value = result_queue.get(timeout=min(1.0, remaining))
                break
            except queue.Empty:
                if self._worker.poll() is not None:
                    raise RuntimeError(
        ...
        if worker.poll() is None:
            try:
                if worker.stdin is not None:
                    worker.stdin.write(json.dumps({"cmd": "shutdown"}) + "\n")
                    worker.stdin.flush()
            except Exception:
                pass
            try:
    ...
    def release_cache(self):
        ...
        if worker is None or worker.poll() is not None:
            return
        self._request_id += 1
        self._worker_request({"id": self._request_id, "cmd": "release_cache"})
    ...
    def generate(self, inputs=None, prompt=None, **kwargs):
        ...
        input_ids_list = _tensor_to_list(input_ids)
        ...
        payload = {
            ...
            "generation_max_length": self.generation_max_length,
            "generation_min_length": self.generation_min_length,
            "do_sample": self.do_sample,
            "temperature": self.temperature,
            "top_p": self.top_p,
