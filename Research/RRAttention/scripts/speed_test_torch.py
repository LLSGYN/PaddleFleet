...
def load_profile_fns(method, enable_profile):
    if method == "xattn":
        import rrattn.modules_torch.xattention as profile_mod
    elif method == "rrattn":
        import rrattn.modules_torch.rrattention as profile_mod
    elif method == "flex":
        import rrattn.modules_torch.flexprefill as profile_mod
    elif method == "full":
        import rrattn.modules_torch.full_prefill as profile_mod
    else:
        raise ValueError(
            f"Unsupported method={method!r}; supported methods are: xattn, rrattn, flex, full"
        )

    profile_mod.set_profile(enable_profile)
    return (
        profile_mod.set_attn_time,
        profile_mod.get_attn_time,
        profile_mod.set_estimate_func_time,
        profile_mod.get_estimate_func_time,
    )
...
def get_prompt_template(model_name):
    ...
    if "llama-3" in lower_name or "llama3" in lower_name:
        return LLAMA3_INSTRUCT_TEMPLATE
    if "qwen" in lower_name:
        return QWEN_INSTRUCT_TEMPLATE
...
def get_kv_retrieval_prompt(data, key, model_name):
    ...
    return get_prompt_template(model_name).format(
...
def quick_get_random_kv_samples(
    ...
    for _ in range(n_sample):
        ...
        prompt = get_kv_retrieval_prompt(ordered_kv_records, key, model_name)
        inputs = tokenizer(
...
def output_name(model_name, method, threshold, stride, output_dir):
    ...
    return (
        Path(output_dir)
        / f"speed_test_torch_{Path(model_name).name}_result_{saved_method}_s{stride}.csv"
    )
...
def main(
    ...
    if not device.startswith("cuda"):
        raise RuntimeError("Torch speed test requires CUDA for flash-attn.")
    ...
    enable_profile = device.startswith("cuda")
    (
        set_attn_time,
        get_attn_time,
        set_estimate_func_time,
        get_estimate_func_time,
    ) = load_profile_fns(method, enable_profile)

    model = load_model(model_name, dtype)
    patch_fn = load_patch(model_type)
    patch_fn(model, method=method, threshold=threshold, stride=stride)
